...
commands = [
    ...
    'utils ha status',  # Reports back the High Availability status, if it is enabled.
    'utils imdb_replication status',  # Reports back the status of IMDB replication.
    # This is not the same as dbreplication.
    'utils diagnose test',  # Built in diagnostics. Output is ugly, remove this line if you don't require this.
    'show perf query counter "Cisco XCP CM" "CmConnectedSockets"',  # Shows the user count for specific node.
    'show perf list instances "Cisco XCP JSM Session Counters"',  # Lists all actual users for specific node. Filter
    # "Composed" and DUPLICATE USERS Rows. Number should match output of "CmConnectedSockets".
    'run pe sql ttlogin select * from clientsessions',  # Reports Jabber Versions being used.
    'run sql select * from enterprisesubcluster',  # IMP Redundancy Group Configuration Table
    'run sql select * from enterprisenode',  # IMP Node Information and Subcluster Assignment.
    'exit'
]
...
def session():
    try:
        ...
        for ipaddr in hostname:
            filename = 'IMPHealthCheck_' + ipaddr.replace('.', '_') + '_' + timestr
