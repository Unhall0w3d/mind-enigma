...
commands = [
    ...
    'show tech call_control_service',  # Pulls a show tech for call control.
    'show db_synchronization status db_ora_config',  # Shows dbsync status for db_ora_config db. Only outputs on Sub.
    'show db_synchronization status db_ora_meta',  # Shows dbsync status for db_ora_meta db. Only outputs on Sub.
    'exit'
]
...
def session():
    try:
        ...
        for ipaddr in hostname:
            filename = 'MSHealthCheck_' + ipaddr.replace('.', '_') + '_' + timestr
